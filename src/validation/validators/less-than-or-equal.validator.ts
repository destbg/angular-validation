...
export function lessThanOrEqualValidator(otherValidControlName: string, groups?: string[], severity?: string): ControlValidator {
    const validator = new ControlValidator({
        identifier: Auth.Ids.lessThanOrEqual,
        groups: groups,
        severity: severity,
    });
