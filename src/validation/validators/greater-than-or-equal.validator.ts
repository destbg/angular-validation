...
export function greaterThanOrEqualValidator(otherValidControlName: string, groups?: string[], severity?: string): ControlValidator {
    const validator = new ControlValidator({
        identifier: Auth.Ids.greaterThanOrEqual,
        groups: groups,
        severity: severity,
    });
